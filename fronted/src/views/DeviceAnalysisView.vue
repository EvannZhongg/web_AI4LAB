...
const gridImportInput = ref(null);
const importSelectionDialog = reactive({
  visible: false,
  rawHeaders: [], // 存储原始表头信息用于展示 { label: '频率', index: 0 }
  rawData: [],    // 存储解析后的完整原始数据
  selectedColumns: [] // 存储用户勾选的列索引 [0, 1, 2]
});
// --- Editor Dialog State ---
...
const reconstructGridData = (experiment) => { /* ... (不变) ... */
  ...
  sortedParams.forEach(param => {
    (param.datapoints || []).forEach(dp => {
      if (!pointsMap[dp.row_index]) {
        pointsMap[dp.row_index] = {};
      }
      pointsMap[dp.row_index][param.id] = dp.value_text;
      if (dp.row_index > maxRow) {
        maxRow = dp.row_index;
      }
    });
  });
...
const gridChartOptions = computed(() => {
  ...
  // --- 现有逻辑：检测 Y 轴数据类型 ---
  const rawYValues = dataRows.map(row => row[yIndex]);
  const numericCount = rawYValues.filter(v => {
    return v !== '' && v !== null && !isNaN(parseFloat(v)) && isFinite(v);
  }).length;
  const isCategoricalY = rawYValues.length > 0 && (numericCount / rawYValues.length < 0.5);

  let seriesData = [];
  let yAxisConfig = {type: 'value', name: headers[yIndex]};
  let categoryMap = {};
  let reverseCategoryMap = {};

  // ... (保留你现有的 isCategoricalY 判断逻辑生成 seriesData) ...
  if (isCategoricalY) {
    const uniqueVals = [...new Set(rawYValues.filter(v => v))];
    uniqueVals.forEach((val, index) => {
      categoryMap[val] = index;
      reverseCategoryMap[index] = val;
    });
    seriesData = dataRows.map(row => {
      const xVal = row[xIndex];
      const rawY = row[yIndex];
      if (!rawY) return null;
      return {
        value: [xVal, categoryMap[rawY]],
        originalY: rawY
      };
    }).filter(item => item !== null);

    yAxisConfig.minInterval = 1;
    yAxisConfig.axisLabel = {formatter: (value) => reverseCategoryMap[value] || value};
  } else {
    seriesData = dataRows
      .map(row => ({
        value: [row[xIndex], parseFloat(row[yIndex])],
        originalY: row[yIndex]
      }))
      .filter(item => item.value[0] != null && !isNaN(item.value[1]));
  }

  // --- 新增逻辑：处理 X 轴排序 ---
  if (isXAxisSorted.value) {
    seriesData.sort((a, b) => {
      const ax = a.value[0];
      const bx = b.value[0];
      // 尝试转为数字进行比较，解决 "1, 10, 2" 的字符串排序问题
      const an = parseFloat(ax);
      const bn = parseFloat(bx);
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        params.forEach(param => {
          const originalVal = param.data.originalY;
          res += `${param.marker} ${param.seriesName}: <b>${originalVal}</b><br/>`;
        });
        return res;
      }
    },
    // 注意：这里 data 使用排序后的 seriesData 生成，保证 X 轴标签顺序正确
    xAxis: {type: 'category', name: headers[xIndex], data: seriesData.map(item => item.value[0])},
    yAxis: yAxisConfig,
...
const waveformChartOptions = computed(() => { /* ... (不变) ... */
  ...
  if (waveform.fetchError) {
    return {
      title: {
        text: `加载 ${csvMeta.filename} 失败`,
        subtext: waveform.fetchError,
        textStyle: {color: 'red'},
        left: 'center',
        top: 'center'
      }
  ...
  if (!waveform.xAxisKey || !waveform.yAxisKey || !waveform.csvData || waveform.csvData.errors?.length > 0) {
    ...
    return {
      title: {
        text: `${experiment.name} - ${csvMeta.filename}`,
        subtext: subtext,
        left: 'center',
        top: 'center'
      }
...
const fetchCsvData = async (experimentId, metadataId) => { // Parameter renamed to metadataId for clarity
  if (!experimentId || !metadataId) {
    waveform.csvData = null;
    waveform.fetchError = null;
    return;
  }
  waveform.loadingCsvData = true;
  waveform.csvData = null;
  waveform.fetchError = null;
  ...
  try {
    ...
    Papa.parse(csvText, {
      ...
      complete: (results) => {
        ...
        if (results.errors.length > 0) {
          ...
          waveform.csvData = results;
        }
        waveform.loadingCsvData = false;
      },
      error: (error) => {
        console.error("PapaParse Error:", error);
        waveform.fetchError = 'CSV 解析失败';
        waveform.loadingCsvData = false;
      }
    });
  } catch (error) {
    ...
    if (error.response?.status === 404) {
      waveform.fetchError = '找不到对应的 CSV 文件 (404)';
    }
    waveform.loadingCsvData = false;
  }
};
...
const loadAnalysisData = async (deviceId) => { /* ... (不变) ... */
  ...
  try {
    ...
    activeExperimentId.value = null;
    photoState.scale = 1;
    photoState.translateX = 0;
    photoState.translateY = 0;
    waveform.activeCsvId = null;
    waveform.availableCsvs = [];
    gridVisualization.xAxisKey = '';
    gridVisualization.yAxisKey = '';
  } catch (error) {
    if (error.response?.status !== 401) {
...
watch(() => props.id, (newId) => { /* ... (不变) ... */
  ...
  if (selectedDeviceId.value !== numId) {
    selectedDeviceId.value = numId;
    loadAnalysisData(numId);
  } else if (numId && !selectedDevice.value) {
    loadAnalysisData(numId);
  } else if (!numId) {
    selectedDevice.value = null;
    experiments.value = [];
...
// --- Event Handlers ---
const onPhotoWheel = (e) => { /* ... (不变) ... */
  photoState.scale = Math.max(0.5, Math.min(5, photoState.scale + (e.deltaY > 0 ? -0.1 : 0.1)));
};
const onPhotoMouseDown = (e) => { /* ... (不变) ... */
...
const triggerGridImport = () => {
  gridImportInput.value.click();
};

const handleGridImportFile = (event) => {
  ...
  reader.onload = (e) => {
    ...
    try {
      if (file.name.endsWith('.csv')) {
        ...
        Papa.parse(file, {
          ...
          complete: (results) => {
            if (results.data && results.data.length > 0) {
              // 确保是二维数组
              updateGridDataFromImport(results.data);
            }
          },
          error: (err) => ElMessage.error('CSV 解析失败: ' + err.message)
        });
      } else {
        // 处理 Excel (.xlsx, .xls)
        ...
        // header: 1 选项会将数据解析为二维数组 [['h1', 'h2'], ['v1', 'v2']]
        const jsonSheet = XLSX.utils.sheet_to_json(worksheet, {header: 1, defval: ''});
        updateGridDataFromImport(jsonSheet);
      }
    } catch (error) {
      console.error(error);
  ...
  if (file.name.endsWith('.csv')) {
    reader.readAsText(file); // PapaParse 需要文本
  } else {
    reader.readAsBinaryString(file); // XLSX 需要二进制字符串 (或 ArrayBuffer)
  }
...
const updateGridDataFromImport = (data) => {
  if (!data || data.length === 0) {
    ElMessage.warning('文件为空');
    return;
  }

  // 1. 数据清洗：将 null/undefined 转为空字符串
  ...
  if (!headers || headers.length === 0) {
    ElMessage.warning('无法识别表头');
    return;
  }

  // 3. 初始化弹窗数据
  importSelectionDialog.rawData = normalizedData;
  // 生成带索引的表头对象，方便 checkbox 绑定
  importSelectionDialog.rawHeaders = headers.map((h, i) => ({
    label: h ? String(h) : `未命名列 ${i + 1}`,
    index: i
  }));
  // 默认全选所有列
  importSelectionDialog.selectedColumns = headers.map((_, i) => i);

  // 4. 打开弹窗
  importSelectionDialog.visible = true;
};

const confirmColumnSelection = () => {
  if (importSelectionDialog.selectedColumns.length === 0) {
    ElMessage.warning('请至少保留一列数据');
    return;
  }

  // 1. 获取排序后的选中索引 (确保列的顺序不乱)
  const sortedIndices = [...importSelectionDialog.selectedColumns].sort((a, b) => a - b);

  // 2. 根据选中的索引过滤数据
  const filteredData = importSelectionDialog.rawData.map(row => {
    // 每一行只保留选中的列
    return sortedIndices.map(index => row[index]);
  });

  // 3. 更新到编辑器表格
  // 提示用户覆盖
  ElMessageBox.confirm(
    `将导入 ${filteredData.length} 行，${sortedIndices.length} 列数据。当前表格内容将被覆盖，确定吗？`,
    '最终确认',
    {type: 'warning'}
  ).then(() => {
    editorDialog.data.grid_data_for_editing = filteredData;
    ElMessage.success('数据导入成功！');
    importSelectionDialog.visible = false;
  }).catch(() => {
  });
};
...
const openEditTableDialog = (experiment) => { /* ... (不变) ... */
  ...
  editorDialog.title = `编辑: ${experiment.name}`;
  editorDialog.data = {
    ...
    csv_files_metadata: JSON.parse(JSON.stringify((experiment.csv_files_metadata || []).map(m => ({
      ...m,
      id: m.metadata_id
    })))) // Use metadata_id as id
  };
  editorDialog.visible = true;
};

const triggerCsvUpload = () => { /* ... (不变) ... */
...
const handleSaveChangesInDialog = async () => {
  if (!editorDialog.data.name) {
    ElMessage.warning('表格名称不能为空！');
    return;
  }
  if (!editorDialog.data.experiment_type) {
    ElMessage.warning('实验类型不能为空！');
    return;
  }
  ...
  const gridDataPayload = { /* ... (不变) ... */
    parameters: editorDialog.data.grid_data_for_editing[0].map((name, index) => ({
      name: name || `参数${index + 1}`,
      column_index: index,
      unit: ''
    })),
    datapoints: editorDialog.data.grid_data_for_editing.slice(1).flatMap((row, rowIndex) => row.map((cell, colIndex) => ({
      parameter_col_index: colIndex,
      row_index: rowIndex,
      value_text: cell ?? ''
    })))
  };
  const csvMetadataPayload = { /* ... (不变) ... */
    csv_files_metadata: editorDialog.data.csv_files_metadata.map(meta => ({
  ...
  try {
    ...
    successfullyUploadedMetaIds.forEach(id => {
      if (pendingCsvFiles[id]) {
        delete pendingCsvFiles[id];
      }
    });
    ...
    if (error.response?.status !== 401) {
      ElMessage.error(`保存失败: ${error.message || '请检查数据或联系管理员'}`);
    }
...
const addColumnInDialog = () => { /* ... (不变) ... */
  ...
  const newHeaderName = `新参数${headerRow.length + 1}`;
  headerRow.push(newHeaderName);
...
const deleteColumnInDialog = (index) => { /* ... (不变) ... */
  ...
  editorDialog.data.grid_data_for_editing.forEach(row => {
    if (row.length > index) {
      row.splice(index, 1);
    }
  });
...
const deleteRowInDialog = (rowIndex) => { /* ... (不变) ... */
  const table = editorDialog.data.grid_data_for_editing;
  if (rowIndex >= 0 && (rowIndex + 1) < table.length && table.length > 2) {
...
<template>
  <div class="device-analysis-page">
    <el-card shadow="never" class="selector-card">
      <el-form :inline="true" @submit.prevent>
        <el-form-item label="器件搜索">
          <el-input v-model="searchQuery" placeholder="名称/编号/技术说明" clearable
                    style="width: 250px;"/>
        </el-form-item>
        <el-form-item label="器件类型">
          <el-select v-model="filterDeviceType" placeholder="所有类型" clearable
                     style="width: 150px;">
        ...
        <el-form-item label="选择器件">
          <el-select v-model="selectedDeviceId" filterable placeholder="请选择"
                     style="width: 300px;" :loading="selectLoading" @change="handleSelectChange">
            <el-option v-for="device in filteredDevicesForSelect" :key="device.id"
                       :label="`${device.name} (${device.device_number})`" :value="device.id"/>
          </el-select>
        </el-form-item>
      </el-form>
    </el-card>
      ...
      <el-row :gutter="20" class="main-content">
        <el-col :span="14" class="full-height-col data-panel">
          <el-card shadow="never" class="photo-card-main">
            ...
            <div class="photo-container" @wheel.prevent="onPhotoWheel"
                 @mousedown.prevent="onPhotoMouseDown" @mousemove.prevent="onPhotoMouseMove"
                 @mouseup="onPhotoMouseUp" @mouseleave="onPhotoMouseUp">
              <el-image v-if="selectedDevice && selectedDevice.photo_data"
                        :src="selectedDevice.photo_data" fit="contain" class="micro-photo"
          ...
          <el-card shadow="never" class="editable-table-card">
            <template #header>
              <div class="card-header"><strong>{{ selectedDevice.name }} 实验数据</strong>
                <el-button type="success" size="small" @click="openAddTableDialog">添加新实验
                </el-button>
              </div>
            </template>
            <div class="custom-tables-container">
              <el-collapse accordion @change="handleTableSelect" v-if="experiments.length > 0">
                <el-collapse-item v-for="experiment in experiments" :key="experiment.id"
                                  :name="experiment.id">
                  <template #title>
                    <div class="collapse-title-wrapper">
                      ...
                      <div class="collapse-actions">
                        <el-button type="primary" link
                                   @click.stop="openEditTableDialog(experiment)">编辑
                        </el-button>
                        <el-button type="danger" link
                                   @click.stop="deleteCustomTable(experiment.id)">删除
                        </el-button>
                      </div>
                    </div>
                  </template>
                  <div class="table-meta"><strong>实验类型:</strong>
                    {{ experiment.experiment_type || '未指定' }}
                  </div>
                  <table class="preview-table"
                         v-if="experiment.grid_data && experiment.grid_data.length > 0">
                    <thead>
                    <tr>
                      <th v-for="(header, index) in experiment.grid_data[0]" :key="`h-${index}`">
                        {{ header }}
                      </th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="(row, rowIndex) in experiment.grid_data.slice(1)"
                        :key="`r-${rowIndex}`">
                    ...
                    <tr v-if="experiment.grid_data.length === 1">
                      <td :colspan="experiment.grid_data[0].length || 1">暂无数据行</td>
                    </tr>
                    </tbody>
                  </table>
                  <div v-else class="table-meta">此实验无表格数据</div>
                  <div
                    v-if="experiment.csv_files_metadata && experiment.csv_files_metadata.length > 0"
                    style="margin-top: 10px;">
                    <strong>关联CSV文件:</strong>
                    <el-tag v-for="csv in experiment.csv_files_metadata"
                            :key="csv.metadata_id || csv.filename" size="small"
                            style="margin-left: 5px;">{{ csv.filename }}
                    </el-tag>
                  </div>
                </el-collapse-item>
              </el-collapse>
        ...
        <el-col :span="10" class="full-height-col visual-panel">
          <el-card shadow="never" class="data-visualization-card">
            <template #header>
              <div class="card-header">
                <strong>数据可视化</strong>
                <div class="waveform-controls" v-if="activeExperimentId">

                  <el-button
                    size="small"
                    :type="isXAxisSorted ? 'primary' : 'default'"
                    @click="isXAxisSorted = !isXAxisSorted"
                    style="margin-right: 10px;"
                  >
                    {{ isXAxisSorted ? 'X轴排序: 开' : 'X轴排序: 关' }}
                  </el-button>

                  <el-select v-model="gridVisualization.xAxisKey" placeholder="X轴" size="small"
                             style="width: 120px;">
                    <el-option v-for="key in gridChartAxisOptions" :key="key" :label="key"
                               :value="key"/>
                  </el-select>
                  <el-select v-model="gridVisualization.yAxisKey" placeholder="Y轴" size="small"
                             style="width: 120px;">
                    <el-option v-for="key in gridChartAxisOptions" :key="key" :label="key"
                               :value="key"/>
                  </el-select>
          ...
          <el-card shadow="never" class="waveform-card">
            <template #header>
              <div class="card-header"><strong>CSV波形窗口</strong>
                <div class="waveform-controls" v-if="activeExperimentId && waveform.activeCsvId">
                  <el-select v-model="waveform.activeCsvId" placeholder="选择CSV文件" size="small"
                             v-if="waveform.availableCsvs.length > 1" style="width: 150px;">
                    <el-option v-for="csv in waveform.availableCsvs" :key="csv.metadata_id"
                               :label="csv.filename" :value="csv.metadata_id"/>
                  </el-select>
                  <el-select v-model="waveform.xAxisKey" placeholder="X轴" size="small"
                             style="width: 100px;">
                    <el-option v-for="col in csvAxisOptions" :key="col.key" :label="col.name"
                               :value="col.key"/>
                  </el-select>
                  <el-select v-model="waveform.yAxisKey" placeholder="Y轴" size="small"
                             style="width: 100px;">
                    <el-option v-for="col in csvAxisOptions" :key="col.key" :label="col.name"
                               :value="col.key"/>
                  </el-select>
                </div>
                <div v-else-if="activeExperimentId && waveform.availableCsvs.length === 0"
                     style="font-size: 12px; color: #909399;"> 当前实验无关联CSV
                </div>
              </div>
            </template>
            <v-chart class="waveform-chart" :option="waveformChartOptions" autoresize
                     v-loading="waveform.loadingCsvData"/>
          </el-card>
        </el-col>
      </el-row>
    </div>
    ...
    <el-dialog v-model="editorDialog.visible" :title="editorDialog.title" width="80%" top="5vh"
               @closed="Object.keys(pendingCsvFiles).forEach(key => delete pendingCsvFiles[key])">
      <el-form :model="editorDialog.data" label-width="100px">
        <el-form-item label="实验名称">
          <el-input v-model="editorDialog.data.name"/>
        </el-form-item>
        <el-form-item label="实验类型">
          <el-input v-model="editorDialog.data.experiment_type" placeholder="例如: 注入试验"/>
        </el-form-item>
      </el-form>
      <el-divider content-position="left">关联的CSV文件元数据</el-divider>
      <div class="csv-list">
        <el-tag v-for="csv in editorDialog.data.csv_files_metadata" :key="csv.metadata_id" closable
                @close="deleteCsvInDialog(csv.metadata_id)">{{ csv.filename }}
        </el-tag>
        <el-button @click="triggerCsvUpload" size="small" type="primary" link>+ 添加CSV元数据
        </el-button>
        <input type="file" id="csv-uploader" @change="handleCsvFileChange" accept=".csv"
               style="display: none;"/>
      </div>
      ...
      <div class="table-controls-dialog">
        ...
        <el-button type="warning" size="small" @click="triggerGridImport"
    ...
    <el-dialog v-model="importSelectionDialog.visible" title="导入设置 - 选择需要的列"
               width="500px">
      <div style="margin-bottom: 15px; color: #606266; font-size: 14px;">
        系统解析到以下列，请勾选您需要导入到表格的列（未勾选的将被忽略）：
      </div>

      <div class="column-selection-container"
           style="max-height: 400px; overflow-y: auto; border: 1px solid #dcdfe6; padding: 10px; border-radius: 4px;">
        <el-checkbox-group v-model="importSelectionDialog.selectedColumns">
          <div v-for="col in importSelectionDialog.rawHeaders" :key="col.index"
               style="margin-bottom: 8px;">
            <el-checkbox :label="col.index">
              <span style="font-weight: bold;">{{ col.label }}</span>
              <span style="color: #909399; font-size: 12px; margin-left: 5px;">(原始第 {{
                  col.index + 1
                }} 列)</span>
            </el-checkbox>
          </div>
      ...
      <div
        style="margin-top: 10px; font-size: 12px; color: #909399; display: flex; justify-content: space-between;">
        <span>共 {{ importSelectionDialog.rawHeaders.length }} 列</span>
        <span>已选 {{ importSelectionDialog.selectedColumns.length }} 列</span>
      </div>

      <template #footer>
        <el-button @click="importSelectionDialog.visible = false">取消</el-button>
        <el-button type="primary" @click="confirmColumnSelection">确认导入</el-button>
      </template>
    </el-dialog>
    <el-dialog v-model="csvMetadataDialog.visible"
               :title="`定义CSV文件列元数据: ${csvMetadataDialog.filename}`" width="60%">
      <p>请为导入的CSV文件 <strong>{{ csvMetadataDialog.filename }}</strong> 的每一列定义名称和单位
        (此操作仅保存元数据)。</p>
      <el-table :data="csvMetadataDialog.columns" border>
        <el-table-column label="原始列" type="index" width="80"/>
        <el-table-column label="数据名称 (Header)">
      ...
      <template #footer>
        <el-button
          @click="csvMetadataDialog.visible = false; csvMetadataDialog.fileToDefine = null;">取消
        </el-button>
        <el-button type="primary" @click="confirmCsvMetadata">确认元数据</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
.photo-card-main :deep(.el-card__body) {
  flex: 1 1 auto;
  min-height: 0;
  padding: 10px;
  display: flex;
...
.editable-table-card :deep(.el-card__body) {
  flex: 1 1 auto;
  min-height: 0;
  padding: 15px;
  display: flex;
...
.table-controls-dialog {
  margin-bottom: 10px;
}

.editable-table-container-dialog {
  max-height: 50vh;
  overflow: auto;
  border: 1px solid #ebeef5;
  margin-bottom: 20px;
...
.editable-table th .el-input, .editable-table td .el-input {
  width: 100%;
}
...
.editable-table th:hover .delete-btn {
  opacity: 1;
}
...
.editable-table td:last-child .delete-btn {
  position: static;
  transform: none;
  opacity: 0.5;
  display: inline-flex;
...
.editable-table td:last-child:hover .delete-btn {
  opacity: 1;
}
...
.editable-table td:not(:last-child) {
  padding: 2px;
}
</style>
